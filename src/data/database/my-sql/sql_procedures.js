...
const insertReservation = (reservation, email) => {
  const numberPeople = reservation.number_people
  const codeHour = reservation.hour.code
  return {
    name: 'SP_INSERTAR_RESERVA_CLIENTE',
    statements: [
      `CALL SP_INSERTAR_RESERVA_CLIENTE("${email}", ${numberPeople}, ${codeHour});`
    ],
    values: []
  }
}
...
const deleteReservation = (id) => {
  return {
    name: 'SP_ELIMINAR_RESERVA_CLIENTE',
    statements: [`CALL SP_ELIMINAR_RESERVA_CLIENTE(${id});`],
    values: []
  }
}
